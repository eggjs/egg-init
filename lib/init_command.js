...
module.exports = class Command {
  ...
  * run() {
    const argv = this.argv = this.getParser().argv;
    ...
    if (!templateDir) {
      ...
      // ask for boilerplate
      let boilerplate;
      if (type && boilerplateMapping.hasOwnProperty(type)) {
        boilerplate = boilerplateMapping[type];
      } else {
  ...
  * askForVariable(targetDir, templateDir) {
    ...
    if (this.argv.silent) {
      this.log('use default due to --silent');
  ...
  * processFiles(targetDir, templateDir) {
    ...
    files.forEach(file => {
      ...
      const to = path.join(targetDir, file === 'gitignore' ? '.gitignore' : file);
  ...
  * getTargetDirectory() {
    ...
    let targetDir = path.resolve(process.cwd(), dir);
    ...
    if (isValid !== true) {
      ...
      const answer = yield this.inquirer.prompt({
        ...
        default: dir || '.',
        filter: dir => path.resolve(process.cwd(), dir),
        validate,
      });
  ...
  * getTemplateDir() {
    ...
    if (this.argv.template) {
      templateDir = path.resolve(process.cwd(), this.argv.template);
  ...
  * fetchBoilerplateMapping(pkgName) {
  ...
  replaceTemplate(content, scope) {
    return content.toString().replace(/{{ *(\w+) *}}/g, (block, key) => {
