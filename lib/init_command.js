...
module.exports = class Command {
  ...
  * fetchBoilerplateMapping(pkgName) {
  ...
  replaceTemplate(content, scope) {
    let replaceTimes = 0;
    const result = content.toString().replace(/(\\)?{{ *(\w+) *}}/g, (block, skip, key) => {
      if (skip) {
        return ++replaceTimes && block.substring(skip.length);
      }
      return scope.hasOwnProperty(key) ? (++replaceTimes && scope[key]) : block;
    });
    return replaceTimes === 0 ? content : result;
  }
