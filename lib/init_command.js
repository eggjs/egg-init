...
module.exports = class Command {
  ...
  * run(cwd, args) {
    ...
    // make bin executable
    this.makeExecutable(this.targetDir);
    // done
  ...
  * processFiles(targetDir, templateDir) {
  ...
  makeExecutable(targetDir) {
    ...
    try {
      const pkgInfo = require(pkgFilePath);
      if (pkgInfo.bin) {
        Object.keys(pkgInfo.bin).forEach(key => {
          const filePath = path.join(targetDir, pkgInfo.bin[key]);
          this.log('chmod 755 to %s', filePath);
          fs.chmodSync(filePath, '755');
        });
      }
    } catch (err) {
      this.log('makeExecutable got error', err.message);
    }
  }
