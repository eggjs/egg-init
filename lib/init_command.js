...
module.exports = class Command {
  ...
  * downloadBoilerplate(pkgName) {
    ...
    const auth = this.getRegistryAuthHeader();
    const response = yield this.curl(tgzUrl, {
  ...
  * curl(url, options) {
  ...
  /**
   * get registry bearer token auth header
   * @return {String|undefined} auth header
   */
  getRegistryAuthHeader() {
    const authInfo = getAuthToken(this.registryUrl);
